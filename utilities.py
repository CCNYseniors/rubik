...
class RubikSolver:
    ...
    def make_bottom_cross(rubik, base=D):
        ...
        cross_edges = [rubik.get_edge(
            colors=(rubik.get_colors(positions=base), c.color)) for c in nbr_c]
    ...
    def make_bottom_corners(rubik, base=D):
        corners = [rubik.get_corner(colors=tuple(
            map(rubik.get_colors, c))) for c in CORNERS if base in c]
        for corner in corners:
            ...
            if base in corner.positions:
                ...
                # Any face would do
                face = corner.positions[0 if corner.positions[1]
                                        == base else 1]
            ...
            while corner.colors != tuple(map(rubik.get_colors, corner.positions)):
                ...
                if face is None:
                    face = next(
                        p for p in positions if p not in corner.positions)
                rubik.move(CW, face)
                rubik.move(ACW, OPPOSITE[base])
                rubik.move(ACW, face)
        for corner in corners:
            assert corner.colors == tuple(
                map(rubik.get_colors, corner.positions))
    ...
    def solve_middle_layer(rubik, base=D):
        ...
        for edge in edges:
            assert edge.colors == tuple(rubik.get_colors(f)
                                        for f in edge.positions)
    ...
    def solve_top_cross(rubik, base=D):
        ...
        edges = list(filter(lambda _edge: up_side in _edge.positions, [
                     rubik.get_edge(positions=e) for e in EDGES]))

        def top_facing_edges(): return list(
        ...
        if len(top_facing_edges()) == 0:
            ...
            some_edge = edges[0]
            right_side = some_edge.positions[0 if some_edge.positions[1]
                                             == up_side else 1]
            rubik.move(CW, up_side)
            front_side = some_edge.positions[0 if some_edge.positions[1]
                                             == up_side else 1]
            rubik.move(ACW, up_side)
        ...
        if len(top_facing_edges()) == 2:
            ...
            edge1_side = facing_edges[0].positions[0 if facing_edges[0].positions[1]
                                                   == up_side else 1]
            edge2_side = facing_edges[1].positions[0 if facing_edges[1].positions[1]
                                                   == up_side else 1]
            ...
            front_side = facing_edges[0].positions[0 if facing_edges[0].positions[1]
                                                   == up_side else 1]
            rubik.move(ACW, up_side)
            algorithm(right_side, up_side, front_side)
    ...
    def solve_top_edges(rubik, base=D):
        ...
        edge1_side = aligned_edges[0].positions[0 if aligned_edges[0].positions[1]
                                                == up_side else 1]
        edge2_side = aligned_edges[1].positions[0 if aligned_edges[1].positions[1]
                                                == up_side else 1]
        if OPPOSITE[edge1_side] == edge2_side:
    ...
    def solve_position_top_corners(rubik, base=D):
        ...
        up_side = OPPOSITE[base]
        up_color = rubik.get_colors(up_side)
        corners = list(filter(lambda cnr: up_color in cnr.colors, [
                       rubik.get_corner(positions=c) for c in CORNERS]))
        ...
        while len(aligned_top_corners()) == 1:
            ...
            side1, side2 = list(
                filter(lambda f: f != up_side, aligned_corner.positions))
            rubik.move(CW, up_side)
